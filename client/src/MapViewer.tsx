...
export class MapViewer extends Component<MapProps, MapState> {
  ...
  // Helper function for rendering a single circle
renderEndpoint = (nearbyFriend : Nearby): JSX.Element => {
  return <circle
    key={`friend-${nearbyFriend.friend}`}
    cx={nearbyFriend.loc.x}
    cy={nearbyFriend.loc.y}
    fill={'orange'}
    r={RADIUS}
    stroke="white"
    strokeWidth={10}
...
renderEndPoints = (): JSX.Element[] => {
  ...
  // Friend circles (single-line arrow function)
  const friendEndpoints = this.state.nearby ? this.state.nearby.map(nearbyFriend => this.renderEndpoint(nearbyFriend)): [];
  return endpoints.concat(friendEndpoints);
    }
  ...
  renderLegendItems = (): Array<JSX.Element> => {
    ...
    this.state.nearby?.map(nearbyFriend => items.push(this.renderLegendItem(nearbyFriend)));
    
    return items;
  };
  
  renderLegendItem = (nearbyFriend : Nearby): JSX.Element => {
    return makeLegendItem('orange', `Friend: ${nearbyFriend.friend}`, 'friend-' + nearbyFriend.friend);
  }

  doHourChange = (evt: ChangeEvent<HTMLSelectElement>): void => {
    const hour = parseHour(evt.target.value);
    this.setState({hour, path: undefined, nearby: undefined});
    console.log("Fetching shortest path for:", this.props.user, "at hour:", hour);
    if (!hour) {
      console.error("No hour selected for shortest path request.");
      return;
    }
...
export const FRIEND_COLORS: Array<string> = [
